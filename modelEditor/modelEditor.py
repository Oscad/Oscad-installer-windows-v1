...
def importEditor(e=None):
  ...
  if temp_model_name.startswith('NMOS',0,4):
    if shutil.copy2(OSCAD_HOME+"/modelLibrary/"+model.modelName,"mos_n.lib"):
    ...
    if shutil.copy2(OSCAD_HOME+"/modelLibrary/"+model.modelName,"mos_p.lib"):
      tkMessageBox.showerror("Import Failed","Unable to import model file "+model.modelName)
    else:
      tkMessageBox.showinfo("Successfully imported","Model file "+model.modelName+" is successfully imported to the project.")
  elif temp_model_name.startswith('D',0,1):
    if shutil.copy2(OSCAD_HOME+"/modelLibrary/"+model.modelName,"1n4007.lib"):
      tkMessageBox.showerror("Import Failed","Unable to import model file "+model.modelName)
    else:
      tkMessageBox.showinfo("Successfully imported","Model file "+model.modelName+" is successfully imported to the project.") 
  else:
    if shutil.copy2(OSCAD_HOME+"/modelLibrary/"+model.modelName,"."):
      tkMessageBox.showerror("Import Failed","Unable to import model file "+model.modelName)
    else:
      tkMessageBox.showinfo("Successfully imported","Model file "+model.modelName+" is successfully imported to the project.") 
...
def exportEditor(e=None):
  ...
  if shutil.copy2(model.modelName,OSCAD_HOME+"/modelLibrary/"):
